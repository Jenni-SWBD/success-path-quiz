...
export default function App() {
  ...
  useEffect(() => {
    ...
    // stash for Results screen + tagging fallback
    try {
...
// =========================================================
// KIT Tagging Helper (runs once double opt-in confirmed)
// =========================================================
...
export default function App() {
  ...
  // Detect KIT redirect (?start=1)
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    if (params.get("start") === "1") setCurrentQuestion(0);
  }, []);
  ...
  // =========================================================
  // GDPR + Double Opt-In Stage
  // =========================================================
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await fetch("https://she-who-builds-different.kit.com/c5d3288b6f", {
        method: "POST",
      ...
      console.error("Error submitting KIT opt-in:", err);
    }
  };
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      if (res.ok) setResendMessage("Confirmation resent ✔️");
      else setResendMessage("Unable to resend, please check your inbox manually.");
    } catch {
      setResendMessage("Something went wrong resending confirmation.");
    } finally {
  ...
  // Show confirmation overlay after opt-in submit
  if (submitted)
    return (
      <div className="confirm-screen">
        <div className="confirm-card">
          <h3>Your quiz link is waiting in your inbox at {email}</h3>
          <p>This is where your Success Path begins.</p>
          <p>
            Didn’t get it?{" "}
            <button
        ...
        <div className="quiz-card">
          ...
          <form onSubmit={handleSubmit}>
            ...
            <label className="checkbox">
              <input type="checkbox" required /> I agree to receive my quiz results and related insights.
            </label>
